...
/**
 * 在筛选弹窗中选择城市
 * 流程：点击左边侧栏"城市" → 点击页面最靠上的"更多" → 进入 areaChoice 页面 → 选择城市 → 保存返回
 */
async function selectCityInFilter(mp, page, cityName) {
  ...
  // 2. 点击页面最靠上的"更多"按钮
  const moreButtons = await page.$$('text')
  let moreClicked = false

  for (const textEl of moreButtons) {
    try {
      const text = await textEl.text()
      if (text === '更多') {
        log(`  点击"更多"按钮`)
  ...
  let targetProvince = cityToProvince[cityName] || cityName

  log(`  [debug] 查找省份: ${targetProvince}`)
  ...
  for (const item of provinceItems) {
    try {
      ...
      if (text === targetProvince || text.startsWith(targetProvince) || text.includes(targetProvince)) {
        await item.tap()
        log(`  OK 点击省份: ${text}`)
        provinceFound = true
        await sleep(TIMEOUTS.FILTER_FILL_GAP)
  ...
  // 如果找不到省份，尝试滚动查找
  if (!provinceFound) {
    log('  [debug] 未找到省份，尝试滚动查找...')
    const scrollview = await areaPage.$('.province-list scroll-view') || await areaPage.$('scroll-view')
    if (scrollview) {
      for (let i = 0; i < 5; i++) {
        try {
          await scrollview.scrollTo(0, 200 * (i + 1))
          await sleep(300)
        } catch {}
      }
      // 重新查找
      const newProvinceItems = await areaPage.$$('.province-item .province-name')
      for (const item of newProvinceItems) {
        try {
          const text = await item.text()
          if (text === targetProvince || text.startsWith(targetProvince) || text.includes(targetProvince)) {
            await item.tap()
  ...
  if (!provinceFound) {
    log(`  [WARN] 找不到省份 "${targetProvince}"，返回`)
    await mp.navigateBack()
    await sleep(TIMEOUTS.NAV_BACK_WAIT)
    return
  }

  // 查找并点击具体城市（尝试原名和添加"市"后缀）
  ...
  for (const item of cityItems) {
    try {
      ...
      if (nameEl) {
        ...
        if (text === cityName || text === cityNameWithSuffix) {
          await item.tap()
          log(`  OK 选择城市: ${text}`)
          citySelected = true
          await sleep(TIMEOUTS.FILTER_FILL_GAP)
  ...
  // 如果找不到具体城市，点击"全部选择"
  if (!citySelected) {
    log(`  [debug] 未找到城市 "${cityName}" 或 "${cityNameWithSuffix}"，点击"全部选择"`)
    for (const item of cityItems) {
      try {
        const nameEl = await item.$('.city-name')
        if (nameEl) {
