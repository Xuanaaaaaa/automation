...
async function waitForSelector(page, selector, timeoutMs = TIMEOUTS.ELEMENT_WAIT, intervalMs = 250) {
...
function hasLoginProfile(value) {
...
async function isLoggedIn(mp) {
  try {
    return hasLoginProfile(await mp.callWxMethod('getStorageSync', 'member'))
  } catch (e) {
    log(`[login] [WARN] read member storage failed: ${e.message}`)
    return false
  }
}

async function waitForLoggedIn(mp, timeoutMs = 8000) {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    if (await isLoggedIn(mp)) return true
    await sleep(500)
  }
  return false
}
...
async function ensureLoggedIn(mp) {
  ...
  // Page path is not enough: logged-out users can still sit on normal pages.
  if (await isLoggedIn(mp)) {
    log(`[login] 已登录，跳过`)
    return
  }
  ...
  if (page.path.includes('phoneLogin')) {
    ...
    log('[login] login state is empty, navigating to phoneLogin...')
    try {
      loginPage = await mp.navigateTo('/pages/login/phoneLogin')
    } catch (e) {
      log(`[login] navigateTo phoneLogin failed: ${e.message}, trying redirectTo...`)
      loginPage = await mp.redirectTo('/pages/login/phoneLogin')
    }
  ...
  const loggedIn = await waitForLoggedIn(mp)
  ...
  if (!loggedIn) {
    ...
    await screenshot(mp, '0-login-failed')
    throw new Error('login failed: member storage is still empty after submitting phone login')
  } else {
...
async function navigateToSearch(mp) {
  ...
  const ready = await waitForSelector(page, '.filter-icon', TIMEOUTS.PAGE_LOAD)
  if (ready) {
    log('  search page ready: .filter-icon found')
  } else {
    log('  [WARN] search page ready selector not found before timeout')
  }
  log(`  OK search page loaded, path: ${page.path}`)
  return page
}

async function navigateToPositionList(mp) {
  log('[step] navigate to idle position list page...')

  try {
    await mp.callWxMethod('setStorageSync', 'positionTabJump', { tab: 'position' })
  } catch (e) {
    log(`  [WARN] set position tab jump failed: ${e.message}`)
  }

  try {
    const page = await mp.switchTab('/pages/position/index')
    await sleep(TIMEOUTS.PAGE_LOAD)
    log(`  OK idle page loaded, path: ${page && page.path}`)
    try {
      await page.callMethod('onShow')
      log('  OK idle position list onShow refresh triggered')
    } catch (e) {
      log(`  [WARN] trigger idle position list refresh failed: ${e.message}`)
    }
    return page
  } catch (e) {
    log(`  [WARN] switchTab position list failed: ${e.message}`)
    throw e
  }
}
...
async function waitForAreaChoicePage(mp) {
  for (let i = 0; i < 20; i++) {
    ...
    if (currentPage && currentPage.path.includes('areaChoice')) {
      const provinceList = await currentPage.$('.province-list')
      const cityList = await currentPage.$('.city-list')
      const saveBtn = await currentPage.$('.save-btn')
      if (provinceList && cityList && saveBtn) return currentPage
    }
    if (i === 0 || (i + 1) % 5 === 0) {
      log(`  [debug] waiting for areaChoice ready... (${i + 1}/20)`)
    }
    await sleep(200)
  }
...
async function waitForLeaveAreaChoicePage(mp, timeoutMs = TIMEOUTS.NAV_BACK_WAIT) {
  const deadline = Date.now() + timeoutMs
  while (Date.now() < deadline) {
    const currentPage = await mp.currentPage()
    if (currentPage && !currentPage.path.includes('areaChoice')) {
...
async function selectCityOnAreaPage(mp, areaPage, cityName) {
  ...
  if (!plan.normalized) {
    ...
    await mp.navigateBack()
    await waitForLeaveAreaChoicePage(mp)
    return false
  }
...
async function selectCityInFilter(mp, page, cityName) {
  ...
  if (!currentPage.path.includes('areaChoice')) {
    await waitForSelector(currentPage, '.confirm-btn', 1500, 100)
  }
}
